...
int main(int argc, char* argv[])
{
	...
	if(argc >= 2)
	{
		...
		if (file_dat.is_open())
		{
			...
			std::vector<Individual> Population(pop_size); // The population 
    		...
    		{                                                                                            
				...
				auto generate = cl::make_kernel<cl::Buffer, cl::Buffer, cl::Buffer, int>(program, "generate_individual");
				...
				d_Population = cl::Buffer(context, CL_MEM_WRITE_ONLY, pop_size * sizeof(Best));
				...
				generate(cl::EnqueueArgs(queue, cl::NDRange(pop_size)), d_F, d_D, d_Population, pop_size);
				queue.finish();
