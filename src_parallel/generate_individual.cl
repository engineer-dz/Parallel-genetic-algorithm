...
void update_X(__global double* d_X, __global int* d_permutation) {
    int size_work_groups = get_local_size(0); // size of a work group (number of items in a work group); number of genes
// The different ids
    int id_global = get_global_id(0);
    int id_local = get_local_id(0);
    int id_group = get_group_id(0);
    ...
    for(int i = 0; i < size_work_groups; i++) {
        d_X[id_group*size_work_groups*size_work_groups + id_local*size_work_groups + i] = 0;
    }
    int permut_id = d_permutation[id_group*size_work_groups + id_local];   // Get permutation as a local ID
    d_X[id_group*size_work_groups*size_work_groups + id_local*size_work_groups + permut_id] = 1; // We fill the positions corresponding to the permutation with 1's
}
...
// The different ids
    ...
    // A = F*X
    for (int j = 0; j < size_work_groups; j++) {
        tmp = 0.0f;
        for (int k = 0; k < size_work_groups; k++) {
            tmp += d_F[id_local*size_work_groups+k] * d_X[id_group*size_work_groups*size_work_groups + k*size_work_groups+j];
        }
        d_A[id_group*size_work_groups*size_work_groups + id_local*size_work_groups+j] = tmp;
    }
    ...
    for (int j = 0; j < size_work_groups; j++) {
        tmp = 0.0f;
        for (int k = 0; k < size_work_groups; k++) {
            tmp += d_X[id_group*size_work_groups*size_work_groups + id_local*size_work_groups+k] * d_D[k*size_work_groups+j];
        }
        d_B[id_group*size_work_groups*size_work_groups + id_local*size_work_groups+j] = tmp;
    }
    ...
    // C = (X*D)transpose
    for(int j = 0; j < size_work_groups; j++)
        d_C[id_group*size_work_groups*size_work_groups + id_local*size_work_groups + j] = d_B[id_group*size_work_groups*size_work_groups + j*size_work_groups + id_local];
    ...
    for (int j = 0; j < size_work_groups; j++) {
        tmp = 0.0f;
        for (int k = 0; k < size_work_groups; k++) {
            tmp += d_A[id_group*size_work_groups*size_work_groups + id_local*size_work_groups+k] * d_C[id_group*size_work_groups*size_work_groups + k*size_work_groups+j];
        }
        d_B[id_group*size_work_groups*size_work_groups + id_local*size_work_groups+j] = tmp;
    }
    ...
    if(id_local == 0) {
        double trace = 0;
        for(int i = 0; i < size_work_groups; i++) {
            trace += d_B[id_group*size_work_groups*size_work_groups + i*size_work_groups + i];
        }
